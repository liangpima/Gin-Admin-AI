...
### P0-3 验证码自动化成本 [Medium]（修正：不能删 `chars`）
...
- **修法**（都是小改动）：
  1. 验证码接口加独立限流：同 IP 生成 ≤ 10 次/分钟（复用 login_guard 的 Redis 计数模式）。
  2. 验证失败**立即作废**该 token（现为 expiry 内可反复重试），成功也作废（一次性）。
  3. 验证码 token 改为绑定客户端指纹（登录请求带上，服务端比对），防止 token 跨端盗用。
- **验收**：同一 token 第二次使用失效；失败后不可重试。
